...
namespace GlmSharpTest.Generated.Vec2
{
    ...
    public class DecimalVec2Test
    {
        ...
        {
            {
                var v = new decvec2(-4.5m);
                Assert.AreEqual(-4.5m, v.x);
                Assert.AreEqual(-4.5m, v.y);
            }
            {
                var v = new decvec2(-2, 6);
                Assert.AreEqual(-2, v.x);
                Assert.AreEqual(6, v.y);
            }
            {
                var v = new decvec2(new decvec2(-3, 7.5m));
                Assert.AreEqual(-3, v.x);
                Assert.AreEqual(7.5m, v.y);
            }
            {
                var v = new decvec2(new decvec3(2.5m, 6.5m, 6));
                Assert.AreEqual(2.5m, v.x);
                Assert.AreEqual(6.5m, v.y);
            }
            {
                var v = new decvec2(new decvec4(9.5m, 7, 3, -1.5m));
